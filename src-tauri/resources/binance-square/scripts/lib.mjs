...
export async function publish(apiKey, body) { const res = await fetch(`${BASE_URL_V1}/content/add`, { method: "POST", headers: { "X-Square-OpenAPI-Key": apiKey, "Content-Type": "application/json", clienttype: "binanceSkill" }, body: JSON.stringify(body) }); if (res.status === 504) return { id: null, shareLink: null }; const json = await res.json(); if (json.code !== "000000") throw new Error(`API error [${json.code}]: ${json.message}`); return json.data; }
